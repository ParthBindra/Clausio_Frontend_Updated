...
export default function IncomeReality({ analysis, rawText, loading }: Props) {
  const suspicious: { title: string; description: string }[] = analysis?.suspiciousPatterns ?? []
  ...
  return (
    ...
    >
      ...
      {!loading && hasData && (
        <>
          {/* Income Cards */}
          {(analysis?.declaredIncome || analysis?.estimatedIncome) && (
            <div
              style={{
                display: 'grid',
            ...
            >
              <IncomeCard title="Declared Income" value={analysis?.declaredIncome ?? '—'} color="#dc2626" background="#fef2f2" />
              <IncomeCard title="Estimated Income" value={analysis?.estimatedIncome ?? '—'} color="#d97706" background="#fff7ed" />
            </div>
          )}
              ...
              {suspicious.map((item, index) => (
                <div
                  ...
                  style={{
                    ...
                    gap: 12,
                    padding: '12px 0',
                    borderBottom: index !== suspicious.length - 1 ? '1px solid #e2e8f0' : 'none',
                  }}
                >
                  ...
                  <div>
                    ...
                    <div style={{ marginTop: 5, color: '#64748b', fontSize: 13, lineHeight: 1.6 }}>{item.description}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ marginTop: 24, background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 12, padding: 16 }}>
            ...
            <div style={{ color: '#334155', fontSize: 14, lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>
              {analysis?.aiInsight ?? rawText ?? 'No insight available.'}
            </div>
          </div>
        </>
      )}
